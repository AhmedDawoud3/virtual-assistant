...
class ModelTest(unittest.TestCase):
    def test_calculator_1(self):
        model = Model()
        self.assertEqual(model.process("3 + 4"), (True, "The answer is 7"))

    def test_calculator_2(self):
        model = Model()
        self.assertEqual(model.process("square root 9"),
                         (True, "The answer is 3"))

    def test_calculator_3(self):
        model = Model()
        self.assertEqual(model.process("9 / 5"), (True, "The answer is 1.800"))

    def test_calculator_division_error(self):
        model = Model()
        self.assertEqual(model.process("9 / 0"),
                         (True, "): Can't divide by zero"))
    ...
    def test_date(self):
        ...
        self.assertEqual(model.process("What is th date"),
                         (True, f"Today is {strftime('%A')} {int(strftime('%d'))} {strftime('%B')} {strftime('%Y')}"))

    def test_time(self):
        model = Model()
        self.assertEqual(model.process("What is the time"),
                         (True, f"It's {int(strftime('%I'))} {int(strftime('%M'))} {strftime('%p')}"))

    def test_pray(self):
        model = Model()
        self.assertRegex(model.process("What is the next pray")[1],
    ...
    def test_get_brightness(self):
        model = Model()
        self.assertEqual(model.process("current brightness")
                         [1], "Current brightness is 0")
